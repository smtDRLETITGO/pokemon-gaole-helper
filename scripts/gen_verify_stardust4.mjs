// gen_verify_stardust4.mjs — 產生星塵第4彈 76 張卡視覺化驗證頁
import { STARDUST_4_CARDS as C } from '../src/data/pokemonDb.cards.stardust4.generated.js';
...
const rows = C.map(c => {
  ...
  const m2 = c.move2Name ? `<br><span style="color:#4fc3f7">↳ ${c.move2Name}</span> <span style="color:#888">${badge(c.move2Type)}</span> <span style="color:#bbb">${c.move2Category==='物理'?'D':'X'}</span>` : '';
...
<h1>星塵第4彈 (cassette/9) · ${total} 張卡驗證</h1>
<div class="sub">資料來源：星等/種族屬性/招式/分類/版型/雙招式 = 背面圖 VLM (OpenRouter 多模態) 逐張讀取；六維 = easyocr 100% OCR；名稱 = 官網 Nuxt payload。D=物理 / X=特殊。</div>
<div class="stats">
  <div class="box"><div class="n">${total}</div><div class="l">總卡數</div></div>
  ...
  <div class="box"><div class="n">${dual} / ${total-dual}</div><div class="l">雙屬性 / 單屬性</div></div>
  <div class="box"><div class="n">${total}/${total}</div><div class="l">正面圖就位</div></div>
...
writeFileSync(resolve(ROOT, 'verify_stardust4.html'), html, 'utf-8');
console.log('→ verify_stardust4.html written ('+C.length+' cards)');
